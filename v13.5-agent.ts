#!/usr/bin/env tsx
/**
 * v13.5-agent.ts - 上下文压缩系统 (~3500行)
 *
 * 核心哲学: "记住重要的，压缩冗余的"
 * ================================================
 * V13.5 在 V13 基础上增加 Context Compression 系统：
 * - 滑动窗口: 保留最近 N 轮完整对话
 * - 智能摘要: 旧对话压缩成摘要
 * - 工具截断: 长输出自动截断
 * - 重要性评分: 按优先级保留内容
...
const BASE_SYSTEM = `你是 OpenClaw V13.5 - 自进化 Agent (带上下文压缩)。

## 工作循环
observe -> route -> heartbeat -> recall -> identify -> plan -> (load claw) -> (delegate -> collect) -> execute -> track -> remember -> reflect -> evolve -> compress
...
if (process.argv[2]) {
  ...
  console.log(`\nOpenClaw V13.5 - 自进化 Agent + 上下文压缩 (${identitySystem.getName()})`);
  console.log(`${memory.stats()} | Claw: ${clawLoader.count} 个 | Evolution: ${evolutionSystem.status().split('\n')[0]}`);
  console.log(`${memory.stats()} | Claw: ${clawLoader.count} 个 | Channels: ${channelManager.status()} | Evolution: active`);
