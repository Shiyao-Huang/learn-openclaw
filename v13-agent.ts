#!/usr/bin/env tsx
/**
 * v13-agent.ts - 自进化系统 (~3000行)
 *
 * 核心哲学: "从数据中学习，持续优化"
 * ================================================
 * V13 在 V12 基础上增加 Evolution 系统：
 * - 行为模式分析: 从内省日志识��模式
 * - 策略自动调整: 根据使用情况优化安全策略
 * - 性能优化: 识别瓶颈并改进
 * - 自修复能力: 检测并修复常见问题
...
const BASE_SYSTEM = `你是 OpenClaw V13 - 自进化 Agent。

## 工作循环
observe -> route -> heartbeat -> recall -> identify -> plan -> (load claw) -> (delegate -> collect) -> execute -> track -> remember -> reflect -> evolve
...
if (process.argv[2]) {
  ...
  console.log(`\nOpenClaw V13 - 自进化 Agent (${identitySystem.getName()})`);
