 ...
 * V2: 本地向量记忆
 * V3: 任务规划系统
 * V4: 子代理协调 (当前) - 在 V3 基础上增加 subagent
 */
...
const TOOLS: Anthropic.Tool[] = [
  ...
  // V4 新增: 子代理工具
  { name: "subagent", description: "委托子任务给隔离的Agent进程执行。适合独立任务如代码审查、模块分析等", input_schema: { type: "object" as const, properties: { task: { type: "string" as const, description: "子任务描述，需明确输入和期望输出" }, context: { type: "string" as const, description: "可选的上下文信息" } }, required: ["task"] } }
];
...
if (process.argv[2]) {
  ...
  console.log("OpenClaw V4 - 子代理协调型 Agent");
