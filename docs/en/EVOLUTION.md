# Learn OpenClaw: V0 -> V9 Evolution (English Mirror)

This folder mirrors the Chinese tutorial structure in `../` with concise English chapters.

## Version matrix (from current code)

| Version | File | Lines | Tools | Focus |
|---|---|---:|---:|---|
| V0 | `v0-agent.ts` | 152 | 1 | Minimal tool loop |
| V1 | `v1-agent.ts` | 310 | 5 | Safe file tools |
| V2 | `v2-agent.ts` | 481 | 10 | Local memory |
| V3 | `v3-agent.ts` | 551 | 11 | Task planning |
| V4 | `v4-agent.ts` | 599 | 12 | Subagents |
| V5 | `v5-agent.ts` | 614 | 12 | Skill loading |
| V5.5 | `v5.5-agent.ts` | 777 | 12 | Hook lifecycle |
| V6 | `v6-agent.ts` | 1041 | 14 | Identity + bootstrap |
| V7 | `v7-agent.ts` | 1406 | 27 | Layered memory |
| V8 | `v8-agent.ts` | 1364 | 31 | Heartbeat |
| V9 | `v9-agent.ts` | 1531 | 35 | Sessions |

## Chapters

- [V0: Bash Is All You Need](./v0-bash-loop.md)
- [V1: Model as Agent](./v1-model-as-agent.md)
- [V2: Local Memory](./v2-local-memory.md)
- [V3: Task Planning](./v3-task-planning.md)
- [V4: Subagent Coordination](./v4-subagent-coordination.md)
- [V5: Skill System](./v5-skill-system.md)
- [V5.5: Hook System](./v5.5-hook-system.md)
- [V6: Identity System](./v6-identity-system.md)
- [V7: Layered Memory](./v7-layered-memory.md)
- [V8: Heartbeat System](./v8-heartbeat-system.md)
- [V9: Session Management](./v9-session-management.md)

## Diff guides

- [V0 -> V1](./evolution/v0-to-v1.md)
- [V1 -> V2](./evolution/v1-to-v2.md)
- [V2 -> V3](./evolution/v2-to-v3.md)
- [V3 -> V4](./evolution/v3-to-v4.md)
- [V4 -> V5](./evolution/v4-to-v5.md)
- [V5 -> V5.5](./evolution/v5-to-v5.5.md)
- [V5.5 -> V6](./evolution/v5.5-to-v6.md)
- [V6 -> V7](./evolution/v6-to-v7.md)
- [V7 -> V8](./evolution/v7-to-v8.md)
- [V8 -> V9](./evolution/v8-to-v9.md)

## Chinese source

- Overview: [../EVOLUTION.md](../EVOLUTION.md)
