#!/usr/bin/env tsx
/**
 * v17-agent/index.ts - OpenClaw V17 模块化入口
 * 
 * V17 新增：外部集成系统
 * - Web 抓取 (web_fetch)
 * - 网页搜索 (web_search)
...
const baseExecutor = createExecutor({
  ...
  introspection,
  skillLoader,
  braveApiKey: config.braveApiKey,  // V17: Web 搜索 API 密钥
});
...
## Web 工具使用
- web_fetch: 抓取网页内容 (支持 markdown/text)
- web_search: Brave 搜索 (需 BRAVE_API_KEY)
...
async function main() {
  ...
  console.log('\x1b[36m║     OpenClaw V17 - 外部集成系统      ║\x1b[0m');
