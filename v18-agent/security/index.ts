...
export class SecuritySystem {
  ...
  // 危险命令模式 (bash 命令内容检查)
  private static DANGEROUS_COMMAND_PATTERNS: Array<{ pattern: RegExp; description: string }> = [
    { pattern: /\brm\s+(-rf?|--recursive)\s+[\/~]/, description: '递归删除根目录或用户目录' },
    { pattern: /\bmkfs\b/, description: '格式化文件系统' },
    { pattern: /\bdd\s+.*of=\/dev\//, description: '直接写入块设备' },
    { pattern: />\s*\/dev\/sd[a-z]/, description: '重定向到块设备' },
    { pattern: /\bcurl\b.*\|\s*(bash|sh|zsh)/, description: '从网络下载并直接执行脚本' },
    { pattern: /\bwget\b.*\|\s*(bash|sh|zsh)/, description: '从网络下载并直接执行脚本' },
    { pattern: /\bchmod\s+777\s+\//, description: '对根目录设置开放权限' },
    { pattern: /\b:(){ :\|:& };:/, description: 'Fork bomb' },
    { pattern: /\/etc\/passwd/, description: '访问系统密码文件' },
    { pattern: /\/etc\/shadow/, description: '访问系统影子密码文件' },
  ];

  // 检查 bash 命令内容是否包含危险模式
  inspectBashCommand(command: string): { safe: boolean; warnings: string[] } {
    const warnings: string[] = [];
    for (const { pattern, description } of SecuritySystem.DANGEROUS_COMMAND_PATTERNS) {
  ...
  checkPermission(tool: string, args: Record<string, any>): {
    ...
    if (tool === 'bash' && args.command) {
      const inspection = this.inspectBashCommand(args.command);
      if (!inspection.safe) {
        // 非 owner 直接拒绝
        if (trustLevel !== 'owner') {
          return {
            allowed: false,
            reason: `检测到危险命令模式: ${inspection.warnings.join('; ')}`
          };
        }
        // owner 也需要确认
        return {
