#!/usr/bin/env tsx
/**
 * v5.5-agent.ts - Hook 基础设施 (~750行)
 *
 * 核心哲学: "在关键时刻插入自定义逻辑"
 * ===================================================
 * V5.5 在 V5 基础上增加 Hook 系统：
 * - 最小 Hook 集: 只实现真正需要的 Hook
 * - bootstrap:files: 动态修改人格文件（soul-switch）
 * - session:start/end: 会话生命周期管理
 *
 * 演进路线:
 * V0: bash 即一切
 * V1: 5个基础工具
 ...
 * V4: 子代理协调
 * V5: Skill 系统
 * V5.5: Hook 基础设施 (当前) - 在 V5 基础上增加 HookSystem
 */
...
// ============================================================================
// V5.5 新增: Hook 系统（最小实现）
// ============================================================================
// 核心哲学: 只实现真正需要的 Hook，不过度设计
// OpenClaw 实际只用了 3 个 Hook: bootstrap:files, command:new/reset, gateway:startup
...
interface HookEvent {
  ...
  prevented: boolean;  // Hook 可以阻止后续处理
}
...
class HookSystem {
  ...
  async emit(type: HookType, context: Record<string, unknown> = {}): Promise<HookEvent> {
    ...
    for (const handler of handlers) {
      await handler(event);
      if (event.prevented) break;  // 如果被阻止，停止后续 Handler
    }
...
// ============================================================================
// V5.5: Bootstrap 文件管理（支持 Hook 修改）
// ============================================================================
...
class BootstrapManager {
  private files: BootstrapFile[] = [];
  private bootstrapDir: string;

  constructor() {
    this.bootstrapDir = path.join(WORKDIR, "bootstrap");
    this.loadFiles();
  }

  private loadFiles(): void {
    if (!fs.existsSync(this.bootstrapDir)) return;

    const entries = fs.readdirSync(this.bootstrapDir).filter(f => f.endsWith(".md"));
    for (const entry of entries) {
      const filePath = path.join(this.bootstrapDir, entry);
      try {
        const content = fs.readFileSync(filePath, "utf-8");
        // 从文件名提取优先级，如 "01-identity.md" -> 1
        const priority = parseInt(entry.match(/^(\d+)-/)?.[1] || "99", 10);
        this.files.push({ path: filePath, content, priority });
      } catch (e) {}
    }

    // 按优先级排序
    this.files.sort((a, b) => a.priority - b.priority);
  }

  // 获取所有 bootstrap 文件（供 Hook 修改）
  getFiles(): BootstrapFile[] {
    return [...this.files];  // 返回副本
  }

  // 设置 bootstrap 文件（Hook 修改后）
  setFiles(files: BootstrapFile[]): void {
    this.files = files;
  }
  ...
  getContent(): string {
    ...
    return this.files.map(f => f.content).join("\n\n---\n\n");
  }
}
...
// 构建系统提示（支持 bootstrap 注入）
function buildSystemPrompt(): string {
  const bootstrapContent = bootstrapManager.getContent();
  const bootstrapSection = bootstrapContent
    ? `\n\n## 🎭 身份与人格\n\n${bootstrapContent}\n`
    : "";

  return `你是 OpenClaw V5.5 - Hook 增强型 Agent，工作目录: ${WORKDIR}
${bootstrapSection}
## 🚨 第一优先级：Skill 加载
...
// 初始化函数：触发 bootstrap:files Hook，构建系统提示
async function initialize(): Promise<void> {
  // 触发 bootstrap:files Hook，允许修改 bootstrap 文件
  if (hooks.has("bootstrap:files")) {
    const files = bootstrapManager.getFiles();
    const event = await hooks.emit("bootstrap:files", { files });
    if (event.context.files) {
      bootstrapManager.setFiles(event.context.files as BootstrapFile[]);
    }
  }
...
// 示例：注册一个 soul-switch Hook（可选，演示用）
// 取消注释以启用邪恶模式
/*
hooks.register("bootstrap:files", async (event) => {
  const files = event.context.files as BootstrapFile[];
  // 替换默认人格为邪恶人格
  const evilSoul = files.find(f => f.path.includes("soul-evil"));
  if (evilSoul) {
    event.context.files = [evilSoul];  // 只保留邪恶人格
    console.log("\x1b[31m[Hook] Soul Switch: 邪恶模式已激活\x1b[0m");
  }
});
*/
...
async function main() {
  ...
  if (process.argv[2]) {
    ...
    console.log("OpenClaw V5.5 - Hook 增强型 Agent (V5 + HookSystem)");
    ...
    if (bootstrapManager.getContent()) console.log(`Bootstrap: 已加载人格文件`);
    console.log("\n输入 'q' 退出\n");
    ask();
  }
}
